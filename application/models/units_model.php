...
class Units_model extends CI_Model {
	...
	public function save_data($project, $revenue_rate, $type, $options, $client_name, $client_address, $client_city, $client_email, $client_contact, $booking_date, $unit_no, $rate, $floor_rise, $plc, $car_park, $car_park_cost, $cashback){
		$agr_value = $options*$rate+$floor_rise+$plc+$car_park_cost;
		$revenue = $agr_value*$revenue_rate*.01;
		$net_revenue = $revenue-$cashback;
		$date = date('Y-m-d', $booking_date);
		$this->db->insert('clients', array('name'=>$client_name, 'date'=>$booking_date, 'email'=>$client_email, 'phone'=>$client_contact, 'city'=>$client_city, 'address'=>$client_address));
		$client_id = $this->db->insert_id();
		$this->db->insert('client_booking', array('client_id' => $client_id, 'project_id'=>$project, 'unit_no'=>$unit_no, 'unit_type'=>$type, 'size'=>$options, 'basic_rate'=>$rate, 'car_park'=>$car_park, 'car_park_cost'=>$car_park_cost, 'floor_rise'=>$floor_rise, 'plc'=>$plc, 'agr_value'=> $agr_value, 'revenue'=>$revenue, 'cashback'=>$cashback, 'net_revenue'=>$net_revenue));
